...
const reportPanic = (message: string, source?: string, lineno?: number) => {
    ...
    if (displayMsg.includes('WebSocket') || displayMsg.includes('StreamLogs') || displayMsg.includes('aistudio-iframe')) return;

    // Is React fully running and is the layout intact right now?
    const root = document.getElementById('root');
    const isReactMountedAndHealthy = !!(window as any).isReactReady && root && root.innerHTML.trim().length > 150;
    ...
    // Only show panic UI for TRUE white screens
    setTimeout(() => {
        const root = document.getElementById('root');
        const isPanicVisible = !!document.getElementById('panic-ui');
        
        // Critical Check: If root element is near-empty after 3s, it's a white screen scenario
...
window.addEventListener('unhandledrejection', (event) => {
    ...
    // Ignore environment noise
    if (reasonText.includes('WebSocket') || reasonText.includes('StreamLogs') || reasonText.includes('aistudio-iframe')) return;

    // Check if React is mounted and running normally
    const root = document.getElementById('root');
    const isReactMounted = !!(window as any).isReactReady && root && root.innerHTML.trim().length > 150;
