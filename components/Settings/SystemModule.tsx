...
const SystemModule: React.FC<Props> = ({ settings, onChange, onOpenManual, onViewSource, onFactoryReset, onExportSettings, onImportSettings }) => {
    ...
    return (
        <div className="p-4 bg-slate-800/30 space-y-5 border-b border-slate-800">
             ...
             {/* 2. Symbol Blacklist Configuration */}
             <div className="pt-2 border-t border-slate-800/60">
                             ...
                             if (e.key === 'Enter') {
                                 ...
                                 if (val) {
                                     ...
                                     if (!currentList.includes(val)) {
                                         onChange('symbolBlacklist', [...currentList, val]);
                                         audioService.speak(`已将 ${val} 加入黑名单`);
                                     }
                                     input.value = '';
                                 }
                             }
                 ...
                 <div className="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto bg-slate-900/40 p-2 rounded border border-slate-800">
                     {(!settings.symbolBlacklist || settings.symbolBlacklist.length === 0) ? (
                         ...
                         settings.symbolBlacklist.map(sym => (
                             <span key={sym} className="inline-flex items-center gap-1 bg-slate-800 border border-slate-700 text-slate-300 font-mono text-[9px] px-1.5 py-0.5 rounded">
                                 ...
                                 <button
                                     onClick={() => {
                                         const newList = (settings.symbolBlacklist || []).filter(s => s !== sym);
