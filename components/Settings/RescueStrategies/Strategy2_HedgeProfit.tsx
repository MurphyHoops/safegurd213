                ...
                <div className="space-y-2 animate-in fade-in">
                    <div className="grid grid-cols-2 gap-2">
                        <div>
                            ...
                            <input type="number" className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1 text-xs text-white" value={Number.isNaN(settings.hedgeOpenRatio) ? '' : settings.hedgeOpenRatio} onChange={(e) => onChange('hedgeOpenRatio', parseFloat(e.target.value))} />
                        </div>
                        <div>
                            <label className="text-[10px] text-slate-500 block mb-1">覆盖盈余阈值 (%)</label>
                            <input type="number" className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1 text-xs text-emerald-400" value={Number.isNaN(settings.hedgeCoverPercent) ? '' : settings.hedgeCoverPercent} onChange={(e) => onChange('hedgeCoverPercent', parseFloat(e.target.value))} />
                        </div>
                    </div>
                    <div className="grid grid-cols-1 gap-2">
                            <div>
                            <label className="text-[10px] text-slate-500 block mb-1">对冲仓位止损 (Hedge SL) %</label>
                            <input type="number" className="w-full bg-slate-800 border border-slate-700 rounded px-2 py-1 text-xs text-red-400" value={Number.isNaN(settings.hedgeProfitClearStopLoss) ? '' : settings.hedgeProfitClearStopLoss} onChange={(e) => onChange('hedgeProfitClearStopLoss', parseFloat(e.target.value))} />
                        </div>
                    </div>
